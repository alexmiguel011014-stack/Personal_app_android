 ...
 * one trainer's own roster (GOALS.md §5e). On GitLive since §18h (was the last user of the
 * official Android SDK); `Query.count()` is the same server-side count aggregation as before.
 */
class AdminViewModel(
    ...
    fun loadUserStats() {
        viewModelScope.launch {
            ...
            _trainerCount.value = runCatching { trainersQuery.count() }.getOrNull()
            _totalUserCount.value = runCatching { firestore.collection("users").count() }.getOrNull()
