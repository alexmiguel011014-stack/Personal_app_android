...
class StudentRepository(
    ...
    // GOALS.md §17e: the student's own users/{uid} doc, live — carries the trainer-granted
    // canSelfAssess/canLogBiometrics flags and the pendingAssessmentRequest signal.
    fun getMyProfile(studentId: String): Flow<UserEntity?> =
        firestore.collection("users").document(studentId).snapshots
            .map { snapshot -> if (snapshot.exists) snapshot.toLinkedUserEntity() else null }
            .catch {
                Firebase.crashlytics.recordException(it)
                emit(null)
            }

    // One batch: the new assessments/{id} doc plus clearing the request flag on the profile.
    // firestore.rules only allows that flag flip when `lastAssessmentId` names a doc that exists
    // after this same batch (existsAfter), which is what ties the two writes together.
    suspend fun submitAssessment(assessment: AssessmentEntity) {
        val batch = firestore.batch()
        batch.set(firestore.collection("assessments").document(assessment.id), assessment.toFirestoreMap())
        batch.updateFields(firestore.collection("users").document(assessment.studentId)) {
            "pendingAssessmentRequest" to false
            "lastAssessmentId" to assessment.id
        }
        batch.commit()
    }

    // GOALS.md §17e: student-logged measurement; firestore.rules gates it on the profile's
    // canLogBiometrics flag (the UI hides the action too, but the rule is the real gate).
